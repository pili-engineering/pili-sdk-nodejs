...
exports.init = function() {
  API.request = new Request(host);
  API.requestRoom = new Request(hostRoom);
};
...
exports.snapshotStream = function(credentials, streamId, name, format, options, fn) {
  ...
  exports.createRoom = function(credentials, ownerId ,roomName, userMax, fn) {
  var path = '/rooms';
  var data = {};
  ...
  if (userMax) {
    data['user_max'] = userMax;
  }
  API.requestRoom.post(credentials, path, data, function(err, responseData) {
    if (err) {
      fn(err, null);
    } else {
      fn(null, responseData);
...
exports.getRoom = function(credentials, roomName, fn) {
  ...
  API.requestRoom.get(credentials, path, null, function(err, responseData) {
    if (err) {
      fn(err, null);
    } else {
...
exports.deleteRoom = function(credentials, roomName, fn) {
  ...
  API.requestRoom.delete(credentials, path, function(err, object) {
    fn(err);
  });
}
