...
Stream.prototype.saveAs = function(name, format, start, end, options, fn) {
  API.saveStreamAs(this.credentials, this.id, name, format, start, end, options, fn);
}
...
Stream.prototype.hlsPlaybackUrls = function(startTime, endTime, fn) {
  var name = fmt.format("%d", Date.now() / 1000);
  this.saveAs(name, null, startTime, endTime, null, function(err, resp) {
      if (err) {
          fn(err, null);
      } else {
          var result = {};
...
Stream.prototype._dynamicUrl = function(rtmpPublishHost) {
  this.nonce = 0;
  var nonce = this._nonceValue();
  var baseURL = fmt.format('%s?nonce=%d', this._baseUrl(rtmpPublishHost), nonce);
  return fmt.format('%s&token=%s', baseURL, this._dynamicToken(baseURL))
}
...
Stream.prototype._dynamicToken = function(rtmpPublishURL) {
  var urlObject = urlUtil.parse(rtmpPublishURL);
