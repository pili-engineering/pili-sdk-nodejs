...
Stream.prototype.rtmpLiveUrl = function(rtmpPlayHost, preset) {
...
Stream.prototype.hlsLiveUrl = function(hlsPlayHost, preset) {
  var url = '';
  if (preset) {
    url = fmt.format('http://%s/%s/%s@%s.m3u8', hlsPlayHost, this.hub, this.title, preset);
  } else {
...
Stream.prototype.hlsPlaybackUrl = function(hlsPlayHost, startTime, endTime, preset) {
  var url = '';
  if (preset) {
